...
import { cashfreeRequest } from './cashfree';
...
export async function finalizeCashfreeOrder(cashfreeOrderId: string): Promise<CashfreePaymentResult> {
  ...
  const internalSnapshot = await adminDb.collection('orders').where('cashfreeOrderId', '==', cashfreeOrderId).get();
  ...
  if (!Number.isFinite(expectedAmount) || Math.abs(cashfreeOrderAmount - expectedAmount) > 0.01) {
    throw new Error('Cashfree payment amount does not match the Seedlings order total.');
  }
  if (successfulPayment && Math.abs(Number(successfulPayment.payment_amount || 0) - expectedAmount) > 0.01) {
    throw new Error('Successful Cashfree transaction amount does not match the Seedlings order total.');
  }

  const subscriptionIds = internalSnapshot.docs
    .map(snapshot => String(snapshot.data().subscriptionId || ''))
    .filter(Boolean);
  const subscriptionSnapshots = await Promise.all(subscriptionIds.map(id => adminDb.collection('subscriptions').doc(id).get()));
  ...
  for (const orderSnapshot of internalSnapshot.docs) {
    ...
    batch.set(adminDb.collection('paymentTransactions').doc(transactionId), {
